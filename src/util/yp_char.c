...
size_t
yp_strspn_whitespace_newlines(const char *string, ptrdiff_t length, yp_allocator_t *allocator, yp_newline_list_t *newline_list, bool stop_at_newline) {
    if (length <= 0) return 0;
    ...
    while (size < maximum && (yp_char_table[(unsigned char) string[size]] & YP_CHAR_BIT_WHITESPACE)) {
        if (string[size] == '\n') {
            if (stop_at_newline) {
                ...
                yp_newline_list_append(allocator, newline_list, string + size);
            }
        }
